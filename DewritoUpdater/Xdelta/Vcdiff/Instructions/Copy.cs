...
namespace Xdelta.Instructions
{
  public class Copy : Instruction
  {
    ...
    private void CopyFromSourceWindow(Window window, Stream input, Stream output)
    {
      ...
      // Decide the source
      var stream = window.Source.Contains(WindowFields.Target) ? output : input;
      ...
      // Get the address
      var address = Address + window.SourceSegmentOffset;
    ...
    {
      var startOutputPosition = stream.Position;
      var availableData = (int) (startOutputPosition - address);
      var buffer = new byte[availableData];

      for (var i = 0; i < length; i += availableData)
      {
        var toCopy = length - i < availableData ? length - i : availableData;
